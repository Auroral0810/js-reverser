...
def encrypt_all_tracedata(token, tracedata, distance):
    """
    对轨迹进行加密
    :param token: token值
    :param tracedata: 轨迹
    ...
    response = requests.get('http://127.0.0.1:4000', data=data)
    data = response.text
    print(data)
    return data
...
class yidun_crack(object):
    ...
    def run(self, ip=None):
        ...
        encrypt_data = encrypt_all_tracedata(yidun.token, tracedata, distance)
        # print(encrypt_data)
