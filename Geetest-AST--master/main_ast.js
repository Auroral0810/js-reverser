...
function replace_name_Array(path) {
    var node = path.node;
    if (node.ccallee == undefined || node.callee.name == undefined)
        return;
    ...
    // 根据参数类型获取值
    let arg;
    if (firstArg.type === "StringLiteral" || firstArg.type === "NumericLiteral") {
        arg = firstArg.value;
    } else if (firstArg.type === "Identifier") {
        // 处理标识符，可能需要在作用域中查找其值
        console.log("警告：参数是标识符，无法直接获取值");
        return;
    } else if (firstArg.type === "MemberExpression") {
        // 处理成员表达式，如 obj.prop
        console.log("警告：参数是成员表达式，无法直接获取值");
        return;
    } else {
        console.log("警告：参数类型不支持 - " + firstArg.type);
        return;
    }
